#!/bin/bash
cat << 'INNER_EOF' > src/components/supplier/SupplierImportView.tsx
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Canonical headers
        const canonical = ['barcode', 'capacity', 'ocv', 'ri', 'gear', 'box_number', 'pallet', 'group', 'manufacturer_name', 'manufacture_date'];
        
        const hFound: Record<string, boolean> = {};
        const hIndex: Record<string, number> = {};
        
        canonical.forEach(ch => {
          const idx = headers.findIndex(h => h === ch || h === ch.replace('_', ' '));
          hFound[ch] = idx !== -1;
          hIndex[ch] = idx;
        });
        ...
        for (let i = 1; i < rawData.length; i++) {
          ...
          if (dateVal instanceof Date) {
            ...
            row.manufacture_date = String(dateVal);
             ...
             row.isValid = false; row.errors.push('Invalid date'); 
          }
    ...
    <div className="flex-1 p-6 space-y-6 overflow-y-auto max-w-7xl mx-auto">
      ...
      {!previewMode && !importResult && (
        <div className="bg-white rounded-2xl shadow-xs border border-slate-200 p-6 space-y-5">
          <h2 className="text-sm font-bold text-slate-900">Select Supplier & Upload</h2>
